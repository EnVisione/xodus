...
use oauth2::{
    ...
    RefreshToken, RequestTokenError, Scope, StandardDeviceAuthorizationResponse,
...
impl XalAuthenticator {
    ...
    pub fn oauth_client(
        ...
        Error,
    > {
        let client = OAuthClient::new(ClientId::new(self.app_params.client_id.to_string()))
            .set_auth_type(AuthType::RequestBody)
            .set_auth_uri(AuthUrl::new(Constants::OAUTH20_AUTHORIZE_URL.to_string())?)
            .set_token_uri(TokenUrl::new(Constants::OAUTH20_TOKEN_URL.to_string())?)
            .set_device_authorization_url(DeviceAuthorizationUrl::new(
                Constants::OAUTH20_DEVICE_AUTHORIZE_URL.to_string(),
            )?);

        let client = if let Some(secret) = self.app_params.client_secret.clone() {
        ...
        SF: std::future::Future<Output = ()>,
    {
        ...
        self.oauth_client()?
            ...
            .request_async(&http_client, sleep_fn, None)
            .await
            .map_err(std::convert::Into::into)
    }
...
mod test {
    ...
    fn implicit_grant_parsing() {
        let redirect_url = Url::parse("https://login.live.com/oauth20_desktop.srf?lc=1033#access_token=access_token_example&token_type=bearer&expires_in=86400&scope=service::user.auth.xboxlive.com::MBI_SSL&refresh_token=refresh_token_example&user_id=test_user&state=XpdMe7g3jH5UZQ").expect("Failed to parse URL");
        ...
        assert_eq!(live_tokens.access_token().secret(), "access_token_example");
        assert_eq!(
            live_tokens.refresh_token().unwrap().secret(),
