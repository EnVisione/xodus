...
A refreshed campaign on August 26, 2026 first reproduced an unchecked FILETIME arithmetic panic from an arbitrary XVD input. The input was replayed from a disposable corpus entry, reduced to deterministic regressions in `crates/msixvc-common` and `crates/msixvc`, and fixed by converting FILETIME parsing to a checked, fallible operation. The rerun at signed checkpoint `6381072` used an exact disposable corpus containing the same 11 tracked fixture files, completed 10,000 executions with final coverage `1644` and feature count `4526`, added 346 non-crash units, reached a peak resident set size of 395 MiB, and reported no crash artifact or sanitizer failure. The disposable corpus and generated artifacts were removed after completion. This evidence covers parser robustness only and does not prove package installation, runtime, or title lifecycle behavior.
