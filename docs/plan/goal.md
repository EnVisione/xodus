...
SRC-051 is verified working-tree evidence, not committed state: typed reversed virtual-extent rejection and `XvdStream` seeks passed formatting, 75 `msixvc` tests, zero-warning workspace Clippy, release build, and filtered 95-test suite. Account test excluded; no unfiltered, package, account, runtime, secret, title-transfer, or external-service validation is claimed.

Continue XODUS-REQ-002 at `SyncSubstream`. Validate `start + len` before exposure and route both NTFS parser construction failures through the public typed boundary. Checked-add `start + pos` for inner read and write targets, reject returned counts above requested bounds, and checked-add `pos` only after bounded I/O succeeds. Return stable `io::ErrorKind` failures before invalid inner I/O or mutation. Test extent and absolute-position overflow, adversarial read and write count rejection without position mutation, and valid bounded read, write, start, current, and end-relative seeks. Defer extraction and HTTP response or retry semantics, other runtime arithmetic, property, fuzz, unsafe-reshape, and static-inventory work. Do not begin XODUS-REQ-003 or Phase 3.
...
Plan SHA-256: 3d04afd9e3c43ab124dc8165c78a0160f50f70b24d6aec43ceea65d3ea554bfc
Validated handoff plan-set SHA-256: ae13504490182fda8d9e2df0cfdb47c0a2ce49a2dc4f6cd9953b577beed4a98c
...
Observed checkout commit: 8b0597505499c95814756bf0b6cceb68585f45f8
...
Remote-head evidence: git ls-remote read-only query observed 2026-08-25 at 09:59:27Z
...
DEC-001 through DEC-012 are locked; optional and future scope is excluded. Preserve tracked work, including SRC-051 and recovery paths. EXT-002A authorizes only Phase 2 entry; broader EXT-002 and later gates remain mandatory. Keep workspace Clippy warning-free without allowances, suppression, lint-level reduction, test weakening, or behavior loss.
