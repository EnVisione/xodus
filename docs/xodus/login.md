...
- **Two kinds of IPC message.** Everything the page posts over IPC lands in `with_ipc_handler` and is tried as two different shapes:
  ...
  - A `HostBridgeMessage` invoking `CloudExperienceHost.getContext` - the page asking its host for context about itself. Xodus answers by JSON encoding the returned context before evaluating a script that calls `dispatchMessage` back into the page with a canned context payload, so webview supplied quotes and script text cannot escape the message.
