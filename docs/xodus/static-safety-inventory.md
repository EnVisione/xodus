...
| --- | --- | --- |
| XVD header and metadata parsing | Typed failures for invalid content type, unsupported XVC versions and keys, bounded XVC and package file counts, bounds, arithmetic, I/O, and allocation. Region-header, encrypted-section, package-file, and segment-file map reservations are fallible before insertion. Invalid content type is rejected before metadata access. | `crates/msixvc/src/xvd.rs`, `XvdFileParseError`, `XvdFileParseError::RegionHeaderAllocationFailed`, `XvdFileParseError::RegionSectionAllocationFailed`, `UserPackageFilesParseError::FileCountTooLarge`, `UserPackageFilesParseError::FileAllocationFailed`, `SegmentMetadataParseError::FileMapAllocationFailed`, `parse_rejects_invalid_content_type_before_metadata_access`, `parse_user_package_files_rejects_oversized_table_before_entry_reads` |
...
| Parser fuzz harness | An explicit `cargo-fuzz` target drives the MSIXVC2, XSP, and XVD parsers from bounded arbitrary bytes without credentials, network access, filesystem writes, or executable loading. The current nightly coverage-guided campaign completed 10,000 executions over 11 tracked MSIXVC2 and XSP fixture files, reached 1,645 inline coverage counters and 4,588 feature counters, added 366 new units, and produced no sanitizer failure or crash artifact. Generated corpus data is disposable and no crash artifact was retained. | `fuzz/fuzz_targets/parse_inputs.rs`; from `fuzz`, `RUSTUP_TOOLCHAIN=nightly cargo fuzz run parse_inputs corpus ../crates/msixvc/testdata/msixvc2 ../crates/msixvc/testdata/xsp -- -runs=10000 -timeout=5 -print_final_stats=1` |
