...
| Account and licensing responses | Typed HTTP, bounded license response and value bodies, schema, empty collection, token conversion, key, license, and UTF 8 failures. Test-only assertions remain outside production paths. | `crates/xodus/src/api`, `crates/xodus/src/models`, `crates/xodus/src/licensing/content.rs`, `oversized_license_response_is_rejected_before_json_decode`, `oversized_license_value_is_rejected_before_decode` |
