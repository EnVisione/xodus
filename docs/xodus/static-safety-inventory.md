...
| --- | --- | --- |
| XVD header and metadata parsing | Typed failures for invalid content type, unsupported XVC versions and keys, bounded XVC and package file counts, bounds, arithmetic, I/O, and allocation. Package-file and segment-file map reservations are fallible before insertion. Invalid content type is rejected before metadata access. | `crates/msixvc/src/xvd.rs`, `XvdFileParseError`, `UserPackageFilesParseError::FileCountTooLarge`, `UserPackageFilesParseError::FileAllocationFailed`, `SegmentMetadataParseError::FileMapAllocationFailed`, `parse_rejects_invalid_content_type_before_metadata_access`, `parse_user_package_files_rejects_oversized_table_before_entry_reads` |
