...
| Account and licensing responses | Typed HTTP, bounded Xbox and display catalog JSON plus license response and value bodies, schema, empty collection, token conversion, key, license, and UTF 8 failures. Test-only assertions remain outside production paths. | `crates/xodus/src/api/xbox/auth.rs`, `crates/xodus/src/api/displaycatalog.rs`, `crates/xodus/src/licensing/content.rs`, `oversized_xbox_response_length_is_rejected_before_json_decode`, `declared_oversized_response_is_rejected`, `oversized_license_response_is_rejected_before_json_decode`, `oversized_license_value_is_rejected_before_decode` |
