...
| Package paths and promotion | Hostile paths fail before output creation, metadata file hashes are bounded before base64 decoding, Linux writes stay beneath the transaction root, and promotion rollback preserves the prior verified state. | `crates/xodus-cli/src/commands/streaming.rs` tests, `crates/xodus-cli/src/commands/download.rs` |
