...
Starting commit: 29baaf573a3b9f6f9ac9b08d619659629d623f90
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 29baaf573a3b9f6f9ac9b08d619659629d623f90
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `29baaf573a3b9f6f9ac9b08d619659629d623f90`, which descends from that merge. The corrected header-directed static PE import probe is the only new evidence considered by this revalidation, and the repository was clean after its disposable directory was deleted. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `29baaf573a3b9f6f9ac9b08d619659629d623f90`; [phase 001 verification](./phase-001-verification.md); [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md); SRC-015 |
...
| End to end game flow | OBSERVED | Authorized login and entitlement-backed discovery completed for both targets, followed by isolated Minecraft package and update-plan metadata inspection, complete Minecraft base-transfer verification against the XVD internal data hash tree, bounded Forza metadata inspection, and successful bounded full Forza transport and hash-tree integrity verification. Both targets have complete-transfer integrity evidence independent of the absent entitlement digests. Corrected memory-only probes reused normal persisted Xodus account state, resolved current entitled metadata, completed protected-content license requests, and decrypted one bounded protected executable only into a Linux memfd per target. The first probe's 16 MiB layout limit yielded no import table. A later header-directed reader accessed only PE header and import sections and superseded that layout limit. Minecraft's static imports contained no observed Game Runtime, online-service, or known anti-cheat import. Forza's static imports contained no observed Game Runtime or known anti-cheat import and did contain an online-service import signal. These are bounded static observations only: absence does not disprove delay-loaded or dynamic behavior, and the Forza signal does not prove actual online behavior. No actual target Game Runtime import evidence, full online-service behavior, anti-cheat classification, runtime compatibility, `xgameruntime` exchange, protected executable launch, installation, save workflow, source-to-target update, or real game execution has passed. | [target package discovery](./target-package-discovery.md); SRC-002; SRC-012; SRC-013; SRC-014; SRC-015 |
...
| Phase 002 entry evidence | OBSERVED | EXT-009's entry artifact contract is complete and available: the eleven-file project-owned synthetic corpus covers the required fixture classes and records exact digests, provenance, rights and license boundaries, compatibility context, and completed security review. No repository parser or transaction test consumes the two new XSP inputs, and no real package apply, rollback, or recovery behavior has passed. EXT-002 remains partial and unavailable. One authorized Minecraft canary transferred the complete 2,490,064,896 byte base package through exact HTTP ranges no larger than 8 MiB; every response passed `206 Partial Content`, exact `Content-Range`, body-length, and stable-total checks. It verified the XVD header `TopHashBlockHash`, every parent hash-tree relationship, and the truncated SHA-256 leaf hash for every 4 KiB data page, then deleted the complete temporary package. Its earlier protected-content license request failed because the Microsoft Store device group was full and yielded no import evidence. A prior bounded Forza base-package inspection retained no content and triggered the existing `XvdFile::parse` seek panic at `crates/msixvc/src/xvd.rs` line 363 under a 128 MiB stream. A separate authenticated metadata-only probe parsed the 4,096 byte XVD header, calculated the XVC metadata offset as 943,964,160 bytes, then showed that a 536,870,912 byte `PrefixCacheFile` cap could not reach that offset: `XvdFile` cached 7,772 bytes and panicked on the bounded seek. A direct authenticated HTTP range request for exactly 4,096 bytes at that offset returned `206 Partial Content` with an exact `Content-Range`, and `XvcInfo` parsed the response in memory as XVC version 2 with 1,964 regions, 1,990 region specifiers, and 21,490 update segments. A bounded 35,920 byte Forza XSP descriptor fetch matched its listed response size and parsed 1,989 records, including 1,986 new-data and 3 copy-data records, for update `3.687.302.0` to `3.688.109.0`, 159,925,977,088 update bytes, and 159,934,996,480 required disk bytes. A metadata-only query found both `FileHash` and `HashOfHashes` absent from both current base-package records. Separate bounded Forza user-metadata ranges supplied only configuration and filename signals. A bounded full Forza canary then completed successfully and stated exactly that it `verified 148 of 148 GiB and passed`, satisfying the full-transfer integrity subcriterion for both targets together with the Minecraft canary. After Microsoft Store device capacity was freed, a corrected disposable memory-only probe reused normal persisted `TokenManager` state without listing or retaining credentials. For each target, current entitled metadata, protected-content license retrieval, and one bounded memfd-only decryption succeeded. Exact ranges no larger than 8 MiB enforced `206 Partial Content`, exact `Content-Range`, and body-length checks. Its first transport attempt stopped cleanly when cache invalidation was found to waste bandwidth; only the corrected bounded run is evidence. The PE import parser was capped at 16 MiB, and neither import table was readable inside that cap. That run's entitlement, license, memfd, transport, and retention evidence remains valid. A later corrected header-directed probe reused the same normal persisted account path and followed the PE layout to access only header and import sections rather than a full executable prefix. Minecraft's static imports contained no observed Game Runtime, online-service, or known anti-cheat import. Forza's static imports contained no observed Game Runtime or known anti-cheat import and did contain an online-service import signal. Static absence does not disprove delay-loaded or dynamic behavior, and the Forza signal does not prove actual online behavior. No credential or raw content was retained. `/tmp/xodus-import-layout-probe.hxp3Zm` was deleted after exit, and the repository was clean before the plan update. These observations prove both target integrity results plus bounded entitlement resolution, protected-content license success, memfd-only decryption, and static import inspection. They do not prove actual Game Runtime imports, runtime compatibility, full online behavior, anti-cheat classification beyond static signals, source-to-target update behavior, installation, saves, launch, game execution, or apply, rollback, and recovery behavior. None closes EXT-002. | SRC-010, SRC-011, SRC-012, SRC-013, SRC-014, SRC-015; [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md); `crates/msixvc/src/xvd.rs`; `docs/cachyos/upstream-overlap.json` PR 136 entry |
...
| EXT-002 | Verified target entitlements and current package metadata | XODUS-REQ-004, XODUS-REQ-014, XODUS-REQ-015, XODUS-REQ-016, XODUS-REQ-019 | unavailable | authorized | Query both target products and freeze entitlement, package, entrypoint, runtime, service, and anti cheat metadata. Preserve the verified integrity, bounded license, and static-import observations while closing actual Game Runtime import, full online-service, anti cheat, and update evidence. |
...
Next executable action: Close only EXT-002 target-metadata evidence by obtaining actual Minecraft and Forza Game Runtime import evidence, full online-service evidence, anti-cheat classification beyond bounded static signals, and real source-to-target update evidence, without beginning XODUS-PHASE-002 implementation.
Known failing checks: Phase 001 baseline verification passed, while Clippy still reports four warnings at the frozen revision. At pre-change implementation branch HEAD 29baaf573a3b9f6f9ac9b08d619659629d623f90, EXT-009 synthetic entry artifact evidence is complete but no XODUS-PHASE-002 parser or transaction consumer tests have run. The Minecraft and Forza full-transfer integrity canaries passed independently. Corrected memory-only probes reused normal persisted Xodus account state without listing credentials and independently completed current entitlement resolution, protected-content license retrieval, and one bounded memfd-only decryption for each target. Exact authenticated ranges no larger than 8 MiB enforced 206 Partial Content, matching Content-Range, and matching body length. The prior 16 MiB probe's evidence remains valid, but its unreadable import-table layout result is superseded by the corrected header-directed reader, which accessed only PE header and import sections. Minecraft's bounded static imports contained no observed Game Runtime, online-service, or known anti-cheat import. Forza's bounded static imports contained no observed Game Runtime or known anti-cheat import and did contain an online-service import signal. Static absence does not disprove delay-loaded or dynamic behavior, and the Forza signal does not prove actual online-service behavior. Neither result proves runtime compatibility, launch, game execution, installation, saves, update, or a source-to-target update. No credential or raw protected content was retained, `/tmp/xodus-import-layout-probe.hxp3Zm` was deleted after exit, and the repository was clean before this plan update. No real package apply or recovery, service-runtime, target-game, performance, Tier 2, or release acceptance run has passed.
