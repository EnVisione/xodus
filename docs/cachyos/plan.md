...
Starting commit: 937ac18c285be27c03eda0daacab10cb2546f05b
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 937ac18c285be27c03eda0daacab10cb2546f05b
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `937ac18c285be27c03eda0daacab10cb2546f05b`, which descends from that merge. The corrected bounded protected-content license and memfd import probe is the only new evidence considered by this revalidation. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `937ac18c285be27c03eda0daacab10cb2546f05b`; [phase 001 verification](./phase-001-verification.md); [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md); SRC-014 |
...
| End to end game flow | OBSERVED | Authorized login and entitlement-backed discovery completed for both targets, followed by isolated Minecraft package and update-plan metadata inspection, complete Minecraft base-transfer verification against the XVD internal data hash tree, bounded Forza update-plan descriptor inspection, limited Forza base-package header and XVC metadata inspection, both-target base integrity-metadata inspection, a bounded Forza unencrypted user-metadata directory and filename-signal scan, and a successful bounded full Forza transport and hash-tree integrity canary. Both targets have complete-transfer integrity evidence independent of the absent entitlement digests. After the owner freed Microsoft Store device capacity, a corrected memory-only probe reused normal persisted `TokenManager` state, resolved current entitled metadata, completed one protected-content license request, and decrypted one bounded protected executable only into a Linux memfd for each target. The 16 MiB PE parser cap could not read either import table. The corrected run retained no package, key, signed URL, decrypted executable, installation, update, launch, or content. No actual target Game Runtime import evidence, actual online-service behavior, anti-cheat classification, runtime compatibility, `xgameruntime` exchange, protected executable launch, save workflow, source-to-target update, or real game execution has passed. | [target package discovery](./target-package-discovery.md); SRC-002; SRC-012; SRC-013; SRC-014 |
...
| EXT-002 | Verified target entitlements and current package metadata | XODUS-REQ-004, XODUS-REQ-014, XODUS-REQ-015, XODUS-REQ-016, XODUS-REQ-019 | unavailable | authorized | Query both target products and freeze entitlement, package, entrypoint, runtime, service, and anti cheat metadata. Preserve the verified integrity and bounded license-request results while closing the remaining protected-PE import, service, anti cheat, and update evidence. |
...
Next executable action: Close only EXT-002 target-metadata evidence by safely obtaining actual Minecraft and Forza protected-PE Game Runtime imports beyond the current 16 MiB parser boundary, actual online-service evidence, anti-cheat classification, and real source-to-target update evidence, without beginning XODUS-PHASE-002 implementation.
Known failing checks: Phase 001 baseline verification passed, while Clippy still reports four warnings at the frozen revision. At pre-change implementation branch HEAD 937ac18c285be27c03eda0daacab10cb2546f05b, EXT-009 synthetic entry artifact evidence is complete but no XODUS-PHASE-002 parser or transaction consumer tests have run. The Minecraft and Forza full-transfer integrity canaries passed independently. The earlier Minecraft protected-content license request failed because the Microsoft Store device group was full, but that capacity result was superseded after the owner freed capacity. One corrected disposable memory-only probe then reused normal persisted TokenManager state without listing or retaining credentials. For Minecraft and Forza independently, current entitled package metadata resolved, the protected-content license request succeeded, and one bounded protected executable was decrypted only into a Linux memfd. Exact HTTP range requests no larger than 8 MiB enforced 206 Partial Content, exact Content-Range, and body-length checks. Its first transport attempt stopped cleanly when cache invalidation was found to waste bandwidth; only the corrected bounded run is evidence. The PE import parser was intentionally capped at 16 MiB, and neither title's import table was readable inside that cap. No package, key, signed URL, decrypted executable, installation, update, launch, or content was retained, and `/tmp/xodus-protected-import-probe.J5yixG` was deleted after exit. This proves bounded entitlement resolution, protected-content license-request success, and memfd-only decryption for both targets, but no actual Game Runtime imports, online-service behavior, anti-cheat classification, runtime compatibility, source-to-target update, installation, or launch. No real package apply or recovery, service-runtime, target-game, performance, Tier 2, or release acceptance run has passed.
