...
Starting commit: f76bae380bf88e03cbb2c8f5600543e3f8ce4968
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: f76bae380bf88e03cbb2c8f5600543e3f8ce4968
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at base revision `f76bae380bf88e03cbb2c8f5600543e3f8ce4968`, which contains the prior XVD package-file payload range slice and descends from that merge. SRC-041 binds the selected working-tree XVD segment path range slice by source and patch digests. This reconciliation records that bounded slice without claiming XODUS-REQ-002 closure, unfiltered-suite success, package activity, runtime activity, or phase completion. | Commands `git rev-parse --abbrev-ref HEAD`, `git rev-parse HEAD`, and `git rev-parse refs/remotes/origin/envy/target-metadata-evidence` identified branch `envy/target-metadata-evidence` and base revision `f76bae380bf88e03cbb2c8f5600543e3f8ce4968`; [phase 001 verification](./phase-001-verification.md); SRC-040; SRC-041 |
...
| Safety and reliability | OBSERVED | The XODUS-REQ-002 package path containment, bounded XSP parser, metadata-directed XVD seek, XVD region-count, XVD key-ID, XVD region-offset lower-bound, XVD fallible allocation, XVD region-end overflow, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, package-file payload range, and segment path range slices now pass their recorded synthetic evidence. A supported region cannot wrap its end or extend beyond the checked declared drive-data end, every hash-entry seek address is checked before reservation and I/O, both page-index sites return the same typed failure on addition overflow, malformed package-file or segment-metadata UTF-16 fails before map insertion, declared metadata length no longer controls reader allocation, and package-controlled segment and package-file tables that exceed their declared extents fail before entry reads. Each package-file payload is bounded by declared user data before filename decoding or insertion, and each segment path is bounded by declared segment metadata before path allocation, seek, decoding, or insertion. Segment hash-slice bounds are the selected next boundary; section-end arithmetic, page-byte arithmetic, other remotely influenced arithmetic, property coverage, and fuzz coverage remain later bounded work. The account-backed test exclusion also prevents a full unfiltered workspace-suite pass. | SRC-002 findings A02, A04, A06, A08; SRC-023 through SRC-041 |
...
| Package formats | OBSERVED | MSIXVC foundations exist, the bounded XSP parser slice rejects hostile count, header-offset, arithmetic, and input-length states before allocation, the selected XVD metadata-directed seek returns a typed I/O failure, XVD region counts above 4,096 fail before allocation, unsupported encrypted XVC key IDs fail through a typed compatibility boundary, encrypted region offsets below user data fail before subtraction, region page vectors use checked conversion with fallible reservation, region-end overflow fails before page work, supported region ends cannot exceed the checked declared drive-data extent, hash-entry seek-address multiplication and additions fail through a typed boundary before reservation or I/O, region hash page-index addition fails through a typed boundary in both preflight and read paths, malformed package-file or segment-metadata UTF-16 fails through concrete parser errors before map insertion, declared metadata length cannot request a package-sized reader buffer, segment-table arithmetic, declared-length bounds, and vector reservation are fallible before entry reads, package-files header and count-derived table extents are checked against declared user data before entry reads, each package-file payload's relative and absolute range is validated before filename decoding or map insertion, and each segment path's relative and absolute range is validated before path-buffer allocation, seek, decoding, or insertion. Segment hash-slice bounds and other hostile downstream boundaries remain open. MSIXVC2 remains unsupported, and XSP parsing still does not provide a complete verified update workflow. | SRC-002 finding A09; SRC-025 through SRC-041 |
| Phase 002 implementation state | OBSERVED | EXT-002A, EXT-008, and EXT-009 remain available and the corrected entry contract remains satisfied. XODUS-PHASE-002 is IN PROGRESS, and its ordered XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, package-file payload range, and segment path range tasks passed their SRC-024 through SRC-041 evidence boundaries. XODUS-REQ-002 remains open because segment hash-slice bounds, section-end and page-byte arithmetic, other downstream arithmetic, unsafe reshape, property, fuzz, and other remotely influenced boundaries remain. Checked segment hash-slice range validation before slice indexing or map insertion is selected as the single next task. The excluded account-backed test remains an explicit verification limitation. Broader EXT-002 and every later blocker remain unchanged. | SRC-010 through SRC-041; [fixture corpus](./fixture-corpus.md); DEC-011 |
...
Current roadmap state is XODUS-PHASE-002 IN PROGRESS. XODUS-PHASE-001 is completed and merged. EXT-002A, EXT-008, and EXT-009 remain available and satisfy the corrected Phase 2 entry contract. The XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, package-file payload range, and segment path range tasks are VERIFIED within SRC-024 through SRC-041. XODUS-REQ-002 remains open, and checked segment hash-slice range validation before slice indexing or map insertion is next. Broader EXT-002 runtime and lifecycle evidence remains PARTIAL and unavailable for later acceptance and stable completion.
...
**Current phase state:** IN PROGRESS through the validated EXT-002A, EXT-008, and EXT-009 entry contract. The XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, package-file payload range, and segment path range tasks are VERIFIED within SRC-024 through SRC-041. XODUS-REQ-002 and XODUS-PHASE-002 remain open, and the excluded account-backed test means no full unfiltered workspace-suite pass is claimed. Broader EXT-002 runtime and lifecycle evidence remains mandatory later but is not an entry dependency.
...
- Next ordered task, XODUS-REQ-002 segment hash-slice range boundary: derive the segment's page-relative hash-slice start and end through checked subtraction, conversion, and addition; validate the range against `section.data_hashs.len()` before slice indexing or map insertion; return typed failures; and prove an out-of-range hash request neither panics nor inserts a record while preserving one valid hash slice through deterministic in-memory evidence. Section-end, page-byte, and other downstream arithmetic remain later bounded tasks.
...
Next executable action: Continue XODUS-REQ-002 in `parse_segment_metadata` by deriving each segment's page-relative hash-slice start and end through checked subtraction, conversion, and addition, validating the range against `section.data_hashs.len()` before slice indexing or map insertion, and returning typed failures. Prove an out-of-range hash request neither panics nor inserts a record while preserving one valid hash slice through deterministic in-memory evidence and no filesystem mutation. Leave section-end, page-byte, and other downstream arithmetic for later bounded tasks. Do not begin XODUS-REQ-003 or XODUS-PHASE-003 until XODUS-REQ-002 closes.
Known failing checks: Phase 1 baseline verification passed. SRC-024 containment, SRC-025 XSP, SRC-026 XVD seek, SRC-027 XVD region-count, SRC-028 XVD key-ID, SRC-029 XVD region-offset, SRC-030 XVD allocation, SRC-031 XVD region-end, SRC-032 XVD declared drive-extent, SRC-033 XVD hash-entry seek-address, SRC-034 XVD hash page-index, SRC-035 package-file UTF-16, SRC-036 segment-metadata UTF-16, SRC-037 segment-metadata fixed reader-capacity, SRC-038 segment-metadata table and allocation, SRC-039 package-files header and table extent, and SRC-040 package-file payload range checks remain passed. For SRC-041, `cargo test -p msixvc` passed 42 tests, targeted `msixvc` Clippy passed with no warnings, CodeGraph synchronization reported one changed file and 189 nodes, `cargo fmt --check`, the workspace release build, workspace Clippy with exactly the four known unrelated warnings, the filtered workspace all-target suite with 62 passing tests, one ignored test, and one filtered test, and `git diff --check` passed. The passing tests comprise 42 `msixvc`, 3 `common`, 9 `xodus`, and 8 `xodus-cli` tests. The warnings remain `chunks_exact` at `crates/xodus/src/licensing/splicense.rs:271` and three `xodus-cli` streaming warnings at lines 299, 373, and 482. The opt-in `api::live::test::test_get_xbox_live_dev_token` remains excluded from offline execution after its prior stall beyond two minutes without network activity, so no unfiltered workspace-suite pass is claimed. XODUS-REQ-002 still lacks segment hash-slice bounds, section-end and page-byte arithmetic, other downstream arithmetic, property, fuzz, unsafe-reshape, and static inventory evidence. EXT-002A remains available only for Phase 2 entry, while broader EXT-002 remains unavailable for later runtime and lifecycle acceptance. EXT-003 and EXT-004 remain unavailable, and no real-package update, target-game, performance, Tier 2, or release acceptance run has passed.
