...
Starting commit: 5f654b0c3f20f8454629063b3dffee544613927f
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 5f654b0c3f20f8454629063b3dffee544613927f
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at base revision `5f654b0c3f20f8454629063b3dffee544613927f`, which contains the prior XVD segment-metadata UTF-16 slice and descends from that merge. SRC-037 binds the selected working-tree XVD segment-metadata fixed reader-capacity slice by source and patch digests. This reconciliation records that bounded slice without claiming XODUS-REQ-002 closure, unfiltered-suite success, package activity, runtime activity, or phase completion. | Commands `git rev-parse --abbrev-ref HEAD`, `git rev-parse HEAD`, and `git rev-parse refs/remotes/origin/envy/target-metadata-evidence` identified branch `envy/target-metadata-evidence` and base revision `5f654b0c3f20f8454629063b3dffee544613927f`; [phase 001 verification](./phase-001-verification.md); SRC-036; SRC-037 |
...
| Safety and reliability | OBSERVED | The XODUS-REQ-002 package path containment, bounded XSP parser, metadata-directed XVD seek, XVD region-count, XVD key-ID, XVD region-offset lower-bound, XVD fallible allocation, XVD region-end overflow, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, and segment-metadata fixed reader-capacity slices now pass their recorded synthetic evidence. A supported region cannot wrap its end or extend beyond the checked declared drive-data end, every hash-entry seek address is checked before reservation and I/O, both page-index sites return the same typed failure on addition overflow, malformed package-file or segment-metadata UTF-16 fails before map insertion, and declared metadata length no longer controls reader allocation. `parse_segment_metadata` still passes package-controlled `segment_count` to `Vec::with_capacity` after unchecked count-derived table arithmetic, allowing a large infallible allocation before segment reads, and that boundary is next. Package-file count and offset validation, segment path arithmetic, hash-slice bounds, other remotely influenced arithmetic, property coverage, and fuzz coverage remain later bounded work. The account-backed test exclusion also prevents a full unfiltered workspace-suite pass. | SRC-002 findings A02, A04, A06, A08; SRC-023 through SRC-037 |
...
| Package formats | OBSERVED | MSIXVC foundations exist, the bounded XSP parser slice rejects hostile count, header-offset, arithmetic, and input-length states before allocation, the selected XVD metadata-directed seek returns a typed I/O failure, XVD region counts above 4,096 fail before allocation, unsupported encrypted XVC key IDs fail through a typed compatibility boundary, encrypted region offsets below user data fail before subtraction, region page vectors use checked conversion with fallible reservation, region-end overflow fails before page work, supported region ends cannot exceed the checked declared drive-data extent, hash-entry seek-address multiplication and additions fail through a typed boundary before reservation or I/O, region hash page-index addition fails through a typed boundary in both preflight and read paths, malformed package-file or segment-metadata UTF-16 fails through concrete parser errors before map insertion, and declared metadata length cannot request a package-sized reader buffer. Package-file count and offset validation, segment-count table and allocation bounds, segment path arithmetic, hash-slice bounds, and other hostile downstream boundaries remain open. MSIXVC2 remains unsupported, and XSP parsing still does not provide a complete verified update workflow. | SRC-002 finding A09; SRC-025 through SRC-037 |
| Phase 002 implementation state | OBSERVED | EXT-002A, EXT-008, and EXT-009 remain available and the corrected entry contract remains satisfied. XODUS-PHASE-002 is IN PROGRESS, and its ordered XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, and segment-metadata fixed reader-capacity tasks passed their SRC-024 through SRC-037 evidence boundaries. XODUS-REQ-002 remains open because package-file count and offsets, segment-count table and allocation bounds, segment path arithmetic, hash-slice bounds, other downstream arithmetic, unsafe reshape, property, fuzz, and other remotely influenced boundaries remain. Typed checked segment-count table validation and fallible reservation are selected as the single next task. The excluded account-backed test remains an explicit verification limitation. Broader EXT-002 and every later blocker remain unchanged. | SRC-010 through SRC-037; [fixture corpus](./fixture-corpus.md); DEC-011 |
...
- Next ordered task, XODUS-REQ-002 segment-count table and allocation boundary: compute the segment table extent with checked arithmetic, reject a table that exceeds declared metadata bounds before entry reads, replace `Vec::with_capacity(segment_count)` with fallible reservation and typed failures, and prove an oversized count cannot allocate or read entries through deterministic in-memory evidence. Package-file count and offset validation, segment path arithmetic, and hash-slice bounds remain later bounded tasks.
