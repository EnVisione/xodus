...
Starting commit: f372b6ea024056bc985cdfa725134a90d1d97742
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: f372b6ea024056bc985cdfa725134a90d1d97742
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at base revision `f372b6ea024056bc985cdfa725134a90d1d97742`, which contains the prior XVD package-files header and table extent slice and descends from that merge. SRC-040 binds the selected working-tree XVD package-file payload range slice by source and patch digests. This reconciliation records that bounded slice without claiming XODUS-REQ-002 closure, unfiltered-suite success, package activity, runtime activity, or phase completion. | Commands `git rev-parse --abbrev-ref HEAD`, `git rev-parse HEAD`, and `git rev-parse refs/remotes/origin/envy/target-metadata-evidence` identified branch `envy/target-metadata-evidence` and base revision `f372b6ea024056bc985cdfa725134a90d1d97742`; [phase 001 verification](./phase-001-verification.md); SRC-039; SRC-040 |
...
Current roadmap state is XODUS-PHASE-002 IN PROGRESS. XODUS-PHASE-001 is completed and merged. EXT-002A, EXT-008, and EXT-009 remain available and satisfy the corrected Phase 2 entry contract. The XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, and package-file payload range tasks are VERIFIED within SRC-024 through SRC-040. XODUS-REQ-002 remains open, and checked segment path range validation before allocation, seek, filename decoding, or map insertion is next. Broader EXT-002 runtime and lifecycle evidence remains PARTIAL and unavailable for later acceptance and stable completion.
...
**Current phase state:** IN PROGRESS through the validated EXT-002A, EXT-008, and EXT-009 entry contract. The XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, segment-metadata UTF-16, segment-metadata fixed reader-capacity, segment-metadata table and allocation, package-files header and table extent, and package-file payload range tasks are VERIFIED within SRC-024 through SRC-040. XODUS-REQ-002 and XODUS-PHASE-002 remain open, and the excluded account-backed test means no full unfiltered workspace-suite pass is claimed. Broader EXT-002 runtime and lifecycle evidence remains mandatory later but is not an entry dependency.
...
- Next ordered task, XODUS-REQ-002 segment path range boundary: compute each path's UTF-16 byte length, relative start and end, and absolute read offset through checked arithmetic; reject a path outside declared segment-metadata length before buffer allocation, seek, filename decoding, or map insertion; return typed failures; and prove overflow or out-of-range paths perform no path read and insert no record while preserving one valid path through deterministic in-memory evidence. Hash-slice bounds remain a later bounded task.
...
Next executable action: Continue XODUS-REQ-002 in `parse_segment_metadata` by computing each segment path's UTF-16 byte length, relative start and end, and absolute read offset through checked arithmetic, validating the path range against declared `segment_metadata.length` before buffer allocation, seek, filename decoding, or map insertion, and returning typed failures. Prove overflow or out-of-range paths perform no path read and insert no record while preserving one valid path through deterministic in-memory evidence and no filesystem mutation. Leave hash-slice bounds and other downstream boundaries for later bounded tasks. Do not begin XODUS-REQ-003 or XODUS-PHASE-003 until XODUS-REQ-002 closes.
Known failing checks: Phase 1 baseline verification passed. SRC-024 containment, SRC-025 XSP, SRC-026 XVD seek, SRC-027 XVD region-count, SRC-028 XVD key-ID, SRC-029 XVD region-offset, SRC-030 XVD allocation, SRC-031 XVD region-end, SRC-032 XVD declared drive-extent, SRC-033 XVD hash-entry seek-address, SRC-034 XVD hash page-index, SRC-035 package-file UTF-16, SRC-036 segment-metadata UTF-16, SRC-037 segment-metadata fixed reader-capacity, SRC-038 segment-metadata table and allocation, and SRC-039 package-files header and table extent checks remain passed. For SRC-040, `cargo fmt`, 40 `msixvc` tests, targeted `msixvc` Clippy with no warnings, CodeGraph synchronization with one changed file and 181 nodes, `cargo fmt --check`, the workspace release build, workspace Clippy with exactly the four known unrelated warnings, the filtered workspace all-target suite with 60 passing tests, one ignored test, and one filtered test, and `git diff --check` passed. The passing tests comprise 40 `msixvc`, 3 `common`, 9 `xodus`, and 8 `xodus-cli` tests. The warnings remain `chunks_exact` at `crates/xodus/src/licensing/splicense.rs:271` and three `xodus-cli` streaming warnings at lines 299, 373, and 482. The opt-in `api::live::test::test_get_xbox_live_dev_token` remains excluded from offline execution after its prior stall beyond two minutes without network activity, so no unfiltered workspace-suite pass is claimed. XODUS-REQ-002 still lacks segment path arithmetic, hash-slice bounds, other downstream arithmetic, property, fuzz, unsafe-reshape, and static inventory evidence. EXT-002A remains available only for Phase 2 entry, while broader EXT-002 remains unavailable for later runtime and lifecycle acceptance. EXT-003 and EXT-004 remain unavailable, and no real-package update, target-game, performance, Tier 2, or release acceptance run has passed.
