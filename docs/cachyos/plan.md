...
Starting commit: 178d755b3f3991d7eaf2b8a567965baba47836b8
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 178d755b3f3991d7eaf2b8a567965baba47836b8
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at base revision `178d755b3f3991d7eaf2b8a567965baba47836b8`, which contains the prior XVD package-file UTF-16 slice and descends from that merge. SRC-036 binds the selected working-tree XVD segment-metadata UTF-16 slice by source and patch digests. This reconciliation records that bounded slice without claiming XODUS-REQ-002 closure, unfiltered-suite success, package activity, runtime activity, or phase completion. | Commands `git rev-parse --abbrev-ref HEAD`, `git rev-parse HEAD`, and `git rev-parse refs/remotes/origin/envy/target-metadata-evidence` identified branch `envy/target-metadata-evidence` and base revision `178d755b3f3991d7eaf2b8a567965baba47836b8`; [phase 001 verification](./phase-001-verification.md); SRC-035; SRC-036 |
...
| Safety and reliability | OBSERVED | The XODUS-REQ-002 package path containment, bounded XSP parser, metadata-directed XVD seek, XVD region-count, XVD key-ID, XVD region-offset lower-bound, XVD fallible allocation, XVD region-end overflow, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, and segment-metadata UTF-16 slices now pass their recorded synthetic evidence. A supported region cannot wrap its end or extend beyond the checked declared drive-data end, every hash-entry seek address is checked before reservation and I/O, both page-index sites return the same typed failure on addition overflow, and malformed package-file or segment-metadata UTF-16 fails before map insertion. `parse_segment_metadata` still derives `BufReader::with_capacity` directly from package-controlled `segment_metadata.length`, allowing a large infallible allocation before parser I/O, and that boundary is next. Package-file count and offset validation, segment count and allocation, segment path arithmetic, hash-slice bounds, other remotely influenced arithmetic, property coverage, and fuzz coverage remain later bounded work. The account-backed test exclusion also prevents a full unfiltered workspace-suite pass. | SRC-002 findings A02, A04, A06, A08; SRC-023 through SRC-036 |
...
| Package formats | OBSERVED | MSIXVC foundations exist, the bounded XSP parser slice rejects hostile count, header-offset, arithmetic, and input-length states before allocation, the selected XVD metadata-directed seek returns a typed I/O failure, XVD region counts above 4,096 fail before allocation, unsupported encrypted XVC key IDs fail through a typed compatibility boundary, encrypted region offsets below user data fail before subtraction, region page vectors use checked conversion with fallible reservation, region-end overflow fails before page work, supported region ends cannot exceed the checked declared drive-data extent, hash-entry seek-address multiplication and additions fail through a typed boundary before reservation or I/O, region hash page-index addition fails through a typed boundary in both preflight and read paths, and malformed package-file or segment-metadata UTF-16 fails through concrete parser errors before map insertion. Package-file count and offset validation, segment-metadata reader capacity and structural bounds, and other hostile downstream boundaries remain open. MSIXVC2 remains unsupported, and XSP parsing still does not provide a complete verified update workflow. | SRC-002 finding A09; SRC-025 through SRC-036 |
| Phase 002 implementation state | OBSERVED | EXT-002A, EXT-008, and EXT-009 remain available and the corrected entry contract remains satisfied. XODUS-PHASE-002 is IN PROGRESS, and its ordered XODUS-REQ-002 path-containment, bounded XSP parser, XVD seek, XVD region-count, XVD key-ID, XVD region-offset, XVD allocation, XVD region-end, XVD declared drive-extent, XVD hash-entry seek-address, XVD hash page-index, package-file UTF-16, and segment-metadata UTF-16 tasks passed their SRC-024 through SRC-036 evidence boundaries. XODUS-REQ-002 remains open because package-file count and offsets, segment-metadata reader capacity and structural bounds, other downstream arithmetic, unsafe reshape, property, fuzz, and other remotely influenced boundaries remain. A fixed package-independent segment-metadata reader capacity is selected as the single next task. The excluded account-backed test remains an explicit verification limitation. Broader EXT-002 and every later blocker remain unchanged. | SRC-010 through SRC-036; [fixture corpus](./fixture-corpus.md); DEC-011 |
...
- Next ordered task, XODUS-REQ-002 segment-metadata reader-capacity boundary: stop passing package-controlled `segment_metadata.length` to infallible `BufReader::with_capacity`; use a small fixed package-independent streaming buffer, preserve valid parsing, and prove a maximum declared metadata length cannot trigger a package-sized allocation before I/O with deterministic in-memory evidence. Package-file count and offset validation, segment count and allocation, segment path arithmetic, and hash-slice bounds remain later bounded tasks.
...
Next executable action: Continue XODUS-REQ-002 by replacing the package-length-derived `BufReader::with_capacity(segment_metadata.length as usize)` in `parse_segment_metadata` with a small fixed package-independent streaming capacity. Preserve valid parsing, and prove a maximum declared metadata length cannot trigger a package-sized allocation before I/O with deterministic in-memory evidence and no filesystem mutation. Leave package-file count and offset validation, segment count and allocation, segment path arithmetic, hash-slice bounds, and other downstream boundaries for later bounded tasks. Do not begin XODUS-REQ-003 or XODUS-PHASE-003 until XODUS-REQ-002 closes.
Known failing checks: Phase 1 baseline verification passed. SRC-024 containment, SRC-025 XSP, SRC-026 XVD seek, SRC-027 XVD region-count, SRC-028 XVD key-ID, SRC-029 XVD region-offset, SRC-030 XVD allocation, SRC-031 XVD region-end, SRC-032 XVD declared drive-extent, SRC-033 XVD hash-entry seek-address, SRC-034 XVD hash page-index, and SRC-035 package-file UTF-16 checks remain passed. For SRC-036, `cargo fmt`, 29 `msixvc` tests, targeted `msixvc` Clippy with no warnings, CodeGraph synchronization with one changed file and 134 nodes, `cargo fmt --check`, the workspace release build, workspace Clippy with exactly the four known unrelated warnings, the filtered workspace all-target suite with 43 passing tests, one ignored test, and one filtered test, and `git diff --check` passed. The warnings remain `chunks_exact` at `crates/xodus/src/licensing/splicense.rs:271` and three `xodus-cli` streaming warnings at lines 299, 373, and 482. The opt-in `api::live::test::test_get_xbox_live_dev_token` remains excluded from offline execution after its prior stall beyond two minutes without network activity, so no unfiltered workspace-suite pass is claimed. XODUS-REQ-002 still lacks package-file count and offset validation, segment-metadata reader capacity and structural bounds, other downstream arithmetic, property, fuzz, unsafe-reshape, and static inventory evidence. EXT-002A remains available only for Phase 2 entry, while broader EXT-002 remains unavailable for later runtime and lifecycle acceptance. EXT-003 and EXT-004 remain unavailable, and no real-package update, target-game, performance, Tier 2, or release acceptance run has passed.
