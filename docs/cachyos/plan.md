...
Starting commit: fb291739e298ac16b81a01b36027e4d0146aa53b
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: fb291739e298ac16b81a01b36027e4d0146aa53b
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `fb291739e298ac16b81a01b36027e4d0146aa53b`, which descends from that merge. This revalidation records the Phase 2 implementation-start state and first selected criterion only. It records no source edit, test result, package action, download, runtime action, or phase completion. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `fb291739e298ac16b81a01b36027e4d0146aa53b`; [phase 001 verification](./phase-001-verification.md); SRC-023 |
...
| Safety and reliability | OBSERVED | CodeGraph shows that `crates/xodus-cli/src/commands/streaming.rs` derives an output path from an untrusted `SegmentFile` job name after replacing backslashes, joins it below `out`, creates parent directories, and opens the file without a containment boundary or symlink-escape defense. No source coverage exercises this path. Package containment, complete integrity validation, CDN fallback, atomic promotion, truthful streaming exit status, malformed input handling, and crash recovery remain incomplete. | SRC-002 findings A02, A04, A06, A08; SRC-023 |
...
| Phase 002 implementation state | OBSERVED | At pre-change revision `fb291739e298ac16b81a01b36027e4d0146aa53b`, EXT-002A, EXT-008, and EXT-009 remain available and the corrected entry contract remains satisfied. Goal validation selected XODUS-REQ-002 package path containment as the first safe implementation criterion, so XODUS-PHASE-002 is IN PROGRESS. The first ordered task rejects unsafe package paths before any creation and protects write operations against symlink escapes using synthetic fixtures and a disposable sandbox. No source file has changed and no implementation test, corpus consumer, real package action, download, stream, secret, or runtime operation is claimed by this state update. Broader EXT-002 and every later blocker remain unchanged. | SRC-010 through SRC-023; [fixture corpus](./fixture-corpus.md); DEC-011 |
...
Current roadmap state is XODUS-PHASE-002 IN PROGRESS. XODUS-PHASE-001 is completed and merged. EXT-002A, EXT-008, and EXT-009 remain available and satisfy the corrected Phase 2 entry contract. Goal validation selected XODUS-REQ-002 package path containment as the first ordered task. Broader EXT-002 runtime and lifecycle evidence remains PARTIAL and unavailable for later acceptance and stable completion.
...
**Current phase state:** IN PROGRESS through the validated EXT-002A, EXT-008, and EXT-009 entry contract. Goal validation selected XODUS-REQ-002 package path containment as the first safe criterion. No source change or passing implementation test is claimed yet. Broader EXT-002 runtime and lifecycle evidence remains mandatory later but is not an entry dependency.

**Implementation scope**

- First ordered task, XODUS-REQ-002 package path containment: reject absolute, drive-prefixed, parent-traversal, ambiguous-separator, and escaping segment paths before directory or file creation; ensure every package write remains inside the disposable transaction root even when existing path components are symlinks; prove the boundary with synthetic fixtures and a disposable filesystem sandbox.
...
Next executable action: Implement only the first XODUS-PHASE-002 criterion in `crates/xodus-cli/src/commands/streaming.rs`: reject unsafe untrusted segment paths before any filesystem creation, prevent package writes from escaping through symlinks, and prove both boundaries with synthetic fixtures in a disposable sandbox before proceeding to later Phase 2 work.
Known failing checks: Phase 1 baseline verification passed, while Clippy still reports four warnings at the frozen revision. At pre-change implementation branch HEAD fb291739e298ac16b81a01b36027e4d0146aa53b, CodeGraph shows that `streaming.rs` joins an untrusted `SegmentFile` job name below `out` after separator replacement, then creates parent directories and opens files without a containment or symlink-escape boundary. No source coverage exercises this path. XODUS-PHASE-002 is IN PROGRESS, but no source edit or passing implementation test is claimed by this plan-only state update. EXT-002 still lacks later target runtime and lifecycle evidence, EXT-003 and EXT-004 artifact manifests remain unavailable, and no real-package, target-game, performance, Tier 2, or release acceptance run has passed.
