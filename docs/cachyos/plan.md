...
Starting commit: 06fc7b61d6b190972561228e073c5fb05af34956
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 06fc7b61d6b190972561228e073c5fb05af34956
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `06fc7b61d6b190972561228e073c5fb05af34956`, which descends from that merge. The complete Minecraft base-transfer integrity canary and the blocked protected-PE license request are the only new evidence considered by this revalidation. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `06fc7b61d6b190972561228e073c5fb05af34956`; [phase 001 verification](./phase-001-verification.md); [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md) |
...
| End to end game flow | OBSERVED | Authorized login and entitlement-backed discovery completed for both targets, followed by isolated Minecraft package and update-plan metadata inspection, complete Minecraft base-transfer verification against the XVD internal data hash tree, bounded Forza update-plan descriptor inspection, limited Forza base-package header and XVC metadata inspection, both-target base integrity-metadata inspection, and a bounded Forza unencrypted user-metadata directory and filename-signal scan. The protected Minecraft PE import probe stopped when Microsoft reported a full device group, so no PE import inspection occurred. Neither current base record exposes a source-supplied digest, and no trustworthy Forza full-transfer integrity result, actual target Game Runtime import evidence, actual online-service behavior, anti-cheat classification, install, `xgameruntime` exchange, protected executable launch, save workflow, source-to-target update, or real game execution has passed. | [target package discovery](./target-package-discovery.md); SRC-002; SRC-012 |
...
| EXT-002 | Verified target entitlements and current package metadata | XODUS-REQ-004, XODUS-REQ-014, XODUS-REQ-015, XODUS-REQ-016, XODUS-REQ-019 | unavailable | authorized | Query both target products and freeze entitlement, package, entrypoint, runtime, service, and anti cheat metadata. Preserve the verified Minecraft base-transfer integrity result and establish trustworthy Forza full-transfer integrity evidence beyond the absent base-record hash metadata. |
...
- EXT-002 is a service prerequisite. Evidence records both product identifiers, current account entitlement, package format, architecture, dependency graph, manifest entrypoint, protected files, Game Runtime imports, online services, update mechanism, and anti cheat classification. Because both current base records lack `FileHash` and `HashOfHashes`, each target requires a trustworthy full-transfer integrity path and completed verification evidence independent of those entitlement fields. The complete Minecraft XVD internal data hash-tree check satisfies the Minecraft portion; a trustworthy Forza full-transfer integrity path and completed verification evidence remain required.
...
EXT-002 still requires trustworthy Forza full-transfer integrity verification beyond current entitlement metadata, actual Minecraft and Forza Game Runtime imports, actual online-service evidence, anti-cheat classification, and real source-to-target update evidence. It remains PARTIAL and unavailable for the XODUS-PHASE-002 entry gate. EXT-009's independent synthetic entry artifact contract is complete and available; it does not satisfy any missing target metadata or any real-package behavior.
