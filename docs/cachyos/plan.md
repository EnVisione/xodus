...
Starting commit: 8d7453a86a025d12099bf6bc3fc5c51fa390234c
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: 8d7453a86a025d12099bf6bc3fc5c51fa390234c
```
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `8d7453a86a025d12099bf6bc3fc5c51fa390234c`, which descends from that merge. The bounded full Forza transport and hash-tree integrity canary is the only new evidence considered by this revalidation. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `8d7453a86a025d12099bf6bc3fc5c51fa390234c`; [phase 001 verification](./phase-001-verification.md); [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md); SRC-013 |
...
| End to end game flow | OBSERVED | Authorized login and entitlement-backed discovery completed for both targets, followed by isolated Minecraft package and update-plan metadata inspection, complete Minecraft base-transfer verification against the XVD internal data hash tree, bounded Forza update-plan descriptor inspection, limited Forza base-package header and XVC metadata inspection, both-target base integrity-metadata inspection, a bounded Forza unencrypted user-metadata directory and filename-signal scan, and a successful bounded full Forza transport and hash-tree integrity canary. The Forza terminal result states exactly that it `verified 148 of 148 GiB and passed`; no package content was written to disk, its exact disposable directory was deleted, and the sleep inhibitor was inactive after exit. Both targets now have complete-transfer integrity evidence independent of the absent entitlement digests. The protected Minecraft PE import probe stopped when Microsoft reported a full device group, and the Forza canary did not inspect a protected PE. No actual target Game Runtime import evidence, actual online-service behavior, anti-cheat classification, protected-content license flow, install, `xgameruntime` exchange, protected executable launch, save workflow, source-to-target update, or real game execution has passed. | [target package discovery](./target-package-discovery.md); SRC-002; SRC-012; SRC-013 |
...
| Phase 002 entry evidence | OBSERVED | EXT-009's entry artifact contract is complete and available: the eleven-file project-owned synthetic corpus covers the required fixture classes and records exact digests, provenance, rights and license boundaries, compatibility context, and completed security review. No repository parser or transaction test consumes the two new XSP inputs, and no real package apply, rollback, or recovery behavior has passed. EXT-002 remains partial and unavailable. One authorized Minecraft canary transferred the complete 2,490,064,896 byte base package through exact HTTP ranges no larger than 8 MiB; every response passed `206 Partial Content`, exact `Content-Range`, body-length, and stable-total checks. It verified the XVD header `TopHashBlockHash`, every parent hash-tree relationship, and the truncated SHA-256 leaf hash for every 4 KiB data page, then deleted the complete temporary package. A subsequent license request made only for protected PE import inspection failed with `Device group is full, please remove a device and try again.` No PE import inspection or other target evidence occurred, and no payload, key, signed URL, or decrypted executable was retained. This closes only the Minecraft full-transfer integrity portion of EXT-002. A prior bounded Forza base-package inspection retained no content and triggered the existing `XvdFile::parse` seek panic at `crates/msixvc/src/xvd.rs` line 363 under a 128 MiB stream. A separate authenticated metadata-only probe parsed the 4,096 byte XVD header, calculated the XVC metadata offset as 943,964,160 bytes, then showed that a 536,870,912 byte `PrefixCacheFile` cap could not reach that offset: `XvdFile` cached 7,772 bytes and panicked on the bounded seek. A direct authenticated HTTP range request for exactly 4,096 bytes at that offset then returned `206 Partial Content` with an exact `Content-Range`, and `XvcInfo` parsed the response in memory as XVC version 2 with 1,964 regions, 1,990 region specifiers, and 21,490 update segments. The response was not written or retained, and no payload, key, signed URL, or decrypted executable was retained. A separate bounded 35,920 byte Forza XSP descriptor fetch matched its listed response size and parsed 1,989 records, including 1,986 new-data and 3 copy-data records, for update `3.687.302.0` to `3.688.109.0`, 159,925,977,088 update bytes, and 159,934,996,480 required disk bytes. Its package-listing FileHash was empty. A separate metadata-only query found both `FileHash` and `HashOfHashes` absent from both current base-package records. It retained no hash value, package response, payload, key, signed URL, or decrypted executable. Separate exact Forza range reads, each no larger than 8 MiB, returned verified `206 Partial Content` and matching `Content-Range` values and parsed only unencrypted user metadata in memory. The six-record directory includes `MicrosoftGame.config`, `AppxManifest.xml`, and `SegmentMetadata.bin`. Neither configuration file contained the inspected `xgameruntime`, `Microsoft.Gaming.Services`, or `GamingServices` markers. The bounded segment-metadata filename inventory contained one online-service signal and no match for the inspected known anti-cheat filename patterns. No response or content was written or retained. A separate bounded full Forza canary completed successfully, with a terminal result stating exactly that it `verified 148 of 148 GiB and passed`. It verified full package transport and hash-tree integrity without writing package content to disk. After exit, `/tmp/xodus-forza-header-probe.SCJJaL` was deleted and the sleep inhibitor was inactive. Together with the Minecraft canary, this satisfies the full-transfer integrity subcriterion for both targets. These observations prove both target integrity results, limited Forza XVC and user-metadata inspection, a prefix-reader boundary, response-size consistency, update-plan parsing, absent base-transfer digests in current entitlement metadata, and only bounded marker and filename signals. They do not prove actual protected-PE imports, Game Runtime compatibility, actual online behavior, anti-cheat presence or classification, source-to-target update behavior, protected-content license flow, target compatibility, or apply, rollback, and recovery behavior. None closes EXT-002. | SRC-010, SRC-011, SRC-012, SRC-013; [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md); `crates/msixvc/src/xvd.rs`; `docs/cachyos/upstream-overlap.json` PR 136 entry |
...
| EXT-002 | Verified target entitlements and current package metadata | XODUS-REQ-004, XODUS-REQ-014, XODUS-REQ-015, XODUS-REQ-016, XODUS-REQ-019 | unavailable | authorized | Query both target products and freeze entitlement, package, entrypoint, runtime, service, and anti cheat metadata. Preserve the verified full-transfer integrity results for both targets while closing the remaining protected-PE import, service, anti cheat, and update evidence. |
...
**Observed protected-PE license boundary:** After the Minecraft integrity check passed, the canary requested the title license only to inspect a protected PE image in memory. Microsoft rejected the request with `Device group is full, please remove a device and try again.` No PE import inspection or other target evidence occurred. No device membership, account setting, installation, update, launch, key, signed URL, decrypted executable, or game payload was retained. This is an external license device-capacity result for the bounded inspection, not evidence about Game Runtime imports, runtime compatibility, online behavior, anti-cheat, or launch behavior.
...
EXT-002 still requires actual Minecraft and Forza protected-PE Game Runtime imports, actual online-service evidence, anti-cheat classification, and real source-to-target update evidence. Neither full-transfer canary supplies protected-PE or license-flow evidence. EXT-002 remains PARTIAL and unavailable for the XODUS-PHASE-002 entry gate. EXT-009's independent synthetic entry artifact contract is complete and available; it does not satisfy any missing target metadata or any real-package behavior.
...
Next executable action: Close only EXT-002 target-metadata evidence by obtaining actual Minecraft and Forza protected-PE Game Runtime imports, actual online-service evidence, anti-cheat classification, and real source-to-target update evidence, without beginning XODUS-PHASE-002 implementation. The protected Minecraft PE import probe remains blocked by Microsoft's device-group capacity response, and neither full-transfer canary proves a protected-content license flow.
Known failing checks: Phase 001 baseline verification passed, while Clippy still reports four warnings at the frozen revision. At pre-change implementation branch HEAD 8d7453a86a025d12099bf6bc3fc5c51fa390234c, EXT-009 synthetic entry artifact evidence is complete but no XODUS-PHASE-002 parser or transaction consumer tests have run. One authorized Minecraft canary transferred the complete 2,490,064,896 byte base package through exact HTTP ranges no larger than 8 MiB. Every response passed 206 Partial Content, exact Content-Range, body-length, and stable-total checks. The canary verified the XVD header TopHashBlockHash, every parent hash-tree relationship, and the truncated SHA-256 leaf hash for every 4 KiB data page, then deleted the complete temporary package. This closes only the Minecraft full-transfer integrity portion of EXT-002. The subsequent license request made only for protected PE import inspection failed with `Device group is full, please remove a device and try again.` No PE import inspection or other target evidence occurred, and no payload, key, signed URL, or decrypted executable was retained. A prior OBSERVED bounded header-only Forza base-package inspection retained no content and panicked in XvdFile::parse at crates/msixvc/src/xvd.rs line 363 after a failed seek under the 128 MiB stream. A separate authenticated metadata-only probe parsed the 4,096 byte XVD header, calculated the XVC metadata offset as 943,964,160 bytes, then showed that a 536,870,912 byte PrefixCacheFile cap could not reach it: XvdFile cached 7,772 bytes and panicked on the bounded seek. A direct authenticated HTTP range request for exactly 4,096 bytes at that offset returned 206 Partial Content with an exact Content-Range, and XvcInfo parsed the response in memory as XVC version 2 with 1,964 regions, 1,990 region specifiers, and 21,490 update segments. The response was not written or retained, and no payload, key, signed URL, or decrypted executable was retained. This closes only the limited Forza XVC metadata inspection and provides no complete parsing, source-to-target update, runtime, online, anti-cheat, apply, rollback, or recovery claim. A separate OBSERVED bounded 35,920 byte Forza XSP descriptor fetch matched the response size and parsed 1,989 records for update 3.687.302.0 to 3.688.109.0, but its empty package-listing FileHash supplies no integrity claim by itself. A separate authenticated metadata-only query found FileHash and HashOfHashes absent from both current base-package records. No hash value, package response, payload, key, signed URL, or decrypted executable was retained. This proves current entitlement metadata cannot bind either complete base transfer to a source-supplied digest and proves no TLS failure or corruption. Minecraft independently passed the complete XVD internal data hash-tree check. A separate bounded full Forza canary completed successfully, with its terminal result stating exactly that it `verified 148 of 148 GiB and passed`. It verified full package transport and hash-tree integrity without writing package content to disk. After exit, `/tmp/xodus-forza-header-probe.SCJJaL` was deleted and the sleep inhibitor was inactive. Together, the canaries satisfy the full-transfer integrity subcriterion for both targets. Separate exact authenticated Forza range reads, each no larger than 8 MiB, returned 206 Partial Content with matching Content-Range values and parsed only unencrypted user metadata in memory. The six-record directory includes MicrosoftGame.config, AppxManifest.xml, and SegmentMetadata.bin. Neither configuration file contained the inspected xgameruntime, Microsoft.Gaming.Services, or GamingServices markers. The bounded segment-metadata filename inventory contained one online-service signal and no match for the inspected known anti-cheat filename patterns. No response or content was written or retained. Neither canary proves protected-PE imports, actual online behavior, Game Runtime compatibility, anti-cheat presence or classification, source-to-target update behavior, or protected-content license flow. No real source-to-target update, package apply or recovery, package install, service-runtime, target-game, performance, Tier 2, or release acceptance run has passed.
