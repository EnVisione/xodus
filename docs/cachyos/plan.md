...
Starting commit: c073512b920114c66ffff2c152901d972f4bc698
Authoritative remote:
origin
https://github.com/EnVisione/xodus.git
Remote ref: origin/envy/target-metadata-evidence
Remote commit: c073512b920114c66ffff2c152901d972f4bc698
```
...
| SRC-012 | audit_evidence | Bounded authorized Forza XSP, base-package XVC metadata, and both-target base integrity-metadata evidence | docs/cachyos/target-package-discovery.md recorded against parser implementation revision 0dca62b2d05b0e2dccd0174ee7b0ceba7a534d86 | Update EXT-002 with exact update-plan and XVC evidence plus absent FileHash and HashOfHashes fields for both current base records, while preserving no complete parsing, full-transfer integrity, corruption, or TLS claim, retained-data limits, and Phase 2 entry block |
...
| --- | --- | --- | --- |
| Repository and phase state | VERIFIED | XODUS-PHASE-001 merged into `main` at `7461e0763907869531b1eb27d444094801a367b4`; signed tag `phase-001-baseline` verifies. The implementation branch and its remote are at pre-change HEAD `c073512b920114c66ffff2c152901d972f4bc698`, which descends from that merge. The both-target base integrity-metadata result is the only new evidence considered by this revalidation. | Commands `git rev-parse HEAD`, `git merge-base --is-ancestor 7461e07 HEAD`, and `git verify-tag phase-001-baseline` exited 0 at pre-change commit `c073512b920114c66ffff2c152901d972f4bc698`; [phase 001 verification](./phase-001-verification.md); [target package discovery](./target-package-discovery.md); [fixture corpus](./fixture-corpus.md) |
...
| End to end game flow | OBSERVED | Authorized login and entitlement-backed discovery completed for both targets, followed by isolated Minecraft package and update-plan metadata inspection, bounded Forza update-plan descriptor inspection, limited Forza base-package header and XVC metadata inspection, and both-target base integrity-metadata inspection. Neither current base record exposes a source-supplied digest, and no complete Forza base-package parsing, install, `xgameruntime` exchange, protected executable launch, save workflow, source-to-target update, full-transfer integrity verification, or real game execution has passed. | [target package discovery](./target-package-discovery.md); SRC-002; SRC-012 |
