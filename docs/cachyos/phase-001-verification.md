...
Checkpoint `1e07267dc34c007e97e2346ae5066c1f91f3b9a2` moves shared file hash decoding into the package metadata boundary. Nonempty hashes must decode from standard or unpadded base64 to exactly 32 bytes before file selection or CDN URL construction, while empty hashes remain accepted. The signed source checkpoint is pushed and passes the focused package validation, file hash, and zero-warning Clippy checks. The complete local verifier also passes `--all`, including formatting, workspace compilation, tests, Clippy, and the release build. The baseline manifest is refreshed to bind this checkpoint. Hosted verification remains pending for this source and documentation checkpoint. This cross-target result will cover the package metadata boundary only and will not establish account, retail package, target runtime, performance, Tier 2, or release completion.
