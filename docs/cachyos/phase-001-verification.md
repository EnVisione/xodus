...
Checkpoint `7d56e1523d8305ac5426f2bde549876241d19179` extends that boundary by rejecting duplicate package file names before selection. The source checkpoint is signed, pushed, and passes the targeted package tests and zero-warning targeted Clippy. Hosted run [32926667811](https://github.com/EnVisione/xodus/actions/runs/32926667811) passed formatting, Linux x86_64, and macOS arm64 checks against documentation checkpoint `80a55d737b3e9da3ec09000ebc3cde5c0622d29c`, which contains the source checkpoint. The baseline manifest is refreshed to bind this checkpoint. This cross-target result covers the package metadata boundary only and does not establish account, retail package, target runtime, performance, Tier 2, or release completion.

Checkpoint `1e07267dc34c007e97e2346ae5066c1f91f3b9a2` moves shared file hash decoding into the package metadata boundary. Nonempty hashes must decode from standard or unpadded base64 to exactly 32 bytes before file selection or CDN URL construction, while empty hashes remain accepted. The signed source checkpoint is pushed and passes the focused package validation, file hash, and zero-warning Clippy checks. The complete local verifier also passes `--all`, including formatting, workspace compilation, tests, Clippy, and the release build. The baseline manifest is refreshed to bind this checkpoint. Hosted run [32927381576](https://github.com/EnVisione/xodus/actions/runs/32927381576) passed formatting, Linux x86_64, and macOS arm64 checks against documentation checkpoint `fbf3687bd974fcb424c7dc9dd491b98e5d03ac1e`, which contains the source checkpoint. This cross-target result covers the package metadata boundary only and does not establish account, retail package, target runtime, performance, Tier 2, or release completion.

The current nightly parser fuzz campaign ran `RUSTUP_TOOLCHAIN=nightly cargo fuzz run parse_inputs <disposable-corpus> -- -runs=10000 -timeout=5 -print_final_stats=1` against branch checkpoint `83a328fe3f59a619dbd24371ef04c6ca09d0bc63`. It loaded all 11 tracked MSIXVC2 and XSP fixtures, completed 10,000 executions, reached 1,623 inline coverage counters and 4,867 feature counters, added 403 new units, and produced no sanitizer failure or crash artifact. The exact disposable corpus directory was removed after the process exited. This fuzz result strengthens parser safety evidence only and does not establish package, account, runtime, target lifecycle, performance, Tier 2, or release completion.
